...
/*struct cash: transaction {
account = "cash"
}*/

class CashViewController: UITableViewController {
	...
	override func viewDidLoad() {
		...
		DispatchQueue.global(qos: .userInteractive).async {
			Service.sharedInstance.runService(choice: "cash", completion: { (data) in
				if (data) {
					self.transactions = Service.sharedInstance.getCash()
				}
		...
		/*DispatchQueue.global(qos: .userInteractive).async {
		self.cashRef.observeSingleEvent(of: .value, with: { snapshot in
		...
		self.transactions.insert(transaction(name: name, details: details, account: "cash", amount: amount, date: date, time: time, timestamp: timestamp), at: 0)
