...
/*struct venmo: transaction {
account = "venmo"
}*/

class VenmoViewController: UITableViewController {
	...
	override func viewDidLoad() {
		...
		DispatchQueue.global(qos: .userInteractive).async {
			Service.sharedInstance.runService(choice: "venmo", completion: { (data) in
				if (data) {
					self.transactions = Service.sharedInstance.getVenmo()
				}
		...
		/*DispatchQueue.global(qos: .userInteractive).async {
		self.venmoRef.observeSingleEvent(of: .value, with: { snapshot in
		...
		self.transactions.insert(transaction(name: name, details: details, account: "venmo", amount: amount, date: date, time: time, timestamp: timestamp), at: 0)
